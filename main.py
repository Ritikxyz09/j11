...
# Bot Token
token = "8504170878:AAEFeQ5bzYkDBRgycgLms61dfFEZy2VN9QA"
...
CHANNEL_LINK = "https://t.me/ritikcyxritik"
GROUP_LINK = "https://t.me/+ajhoCnHPPeU5MDU1"
...
def approve_user(message):
    if message.from_user.id not in admins and message.from_user.id != owner_id:
        bot.reply_to(message, "🚫 Only admins can approve users DM: @Ritikxyz099.")
        return
    parts = message.text.split()
    if len(parts) != 2:
...
def remove_user(message):
    if message.from_user.id not in admins and message.from_user.id != owner_id:
        bot.reply_to(message, "🚫 Only admins can remove users DM @Ritikxyz099.")
        return
    parts = message.text.split()
    if len(parts) != 2:
...
def remove_all_users(message):
    if message.from_user.id not in admins and message.from_user.id != owner_id:
        bot.reply_to(message, "🚫 Only admins can clear approved users DM @Ritikxyz099.")
        return
...
def list_approved(message):
    if message.from_user.id not in admins and message.from_user.id != owner_id:
        bot.reply_to(message, "🚫 Only admins can view approved users DM @Ritikxyz099.")
        return
...
def send_all_galis(message):
    ...
    if message.from_user.id not in [u['id'] for u in approved_users] and message.from_user.id not in admins and message.from_user.id != owner_id:
        bot.reply_to(message, "🚫 You are not approved to use this command DM @Ritikxyz099.")
        return
    parts = message.text.split()
    if len(parts) != 2:
